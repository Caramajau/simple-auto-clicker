...
class TextView:
    def __init__(self) -> None:
        ...
        EventSystem.add_listener(
            Events.TOGGLE_RECORDING, self.__handle_toggle_recording
        )
        EventSystem.add_listener(
            Events.RECORD_MOUSE_CLICK, self.__handle_record_mouse_click
        )
        EventSystem.add_listener(
            Events.CLEAR_MOUSE_POSITIONS, self.__handle_clear_mouse_positions
        )
        EventSystem.add_listener(
            Events.START_MOUSE_CLICKING, self.__handle_start_mouse_clicking
        )
        EventSystem.add_listener(
            Events.STOP_MOUSE_CLICKING, self.__handle_stop_mouse_clicking
        )

    def __handle_program_start(
        self,
        toggle_recording_key: str,
        record_mouse_click_key: str,
    ...
    def __handle_record_mouse_click(
        self, position_to_add: tuple[int, int], all_positions: list[tuple[int, int]]
    ) -> None:
    ...
    def __handle_start_mouse_clicking(
    ...
    def __handle_stop_mouse_clicking(self, start_key: str) -> None:
        print(
            f"Stopped clicking recorded positions, press {start_key} to start the clicking again."
        )
